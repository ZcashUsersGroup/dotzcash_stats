import type { WaitlistReferralRow } from "./referral-dashboard.js";
...
export function buildDailyRankingsFromRows(
  ...
  const nameMap = buildNameMap(orderedRows);
  const preferredCodeMap = buildPreferredCodeMap(orderedRows);
